...
- Repository orchestration entrypoints should go through `bun run` so Windows, WSL, and Linux share one operator surface. Keep the heavy training and evaluation logic inside Python scripts behind that entrypoint rather than re-implementing control flow in shell.
